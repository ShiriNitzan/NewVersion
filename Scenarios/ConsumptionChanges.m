 function [ElectricityConsumptionTable, TransportationConsumptionTable, VehicleAmountsCell, FoodConsumptionCell, WaterConsumptionCell, ConstructionTable,WateAndRecyclingCell, AmountsOfFuelsCells, OrganicWasteAmount] = ConsumptionChanges(Data, PrecentegeByTheYears,Years,pop,orderIndex, varargin)
...
if (PrecentegeByTheYears(1) == PrecentegeByTheYears(width(PrecentegeByTheYears))) && (orderIndex == 2) %The essential change that ensures the correctness of the water mod in a single scenario
    for i =2:Years
        CurrentConsumption = array2table(zeros(1,8), 'VariableNames', ColNames);
